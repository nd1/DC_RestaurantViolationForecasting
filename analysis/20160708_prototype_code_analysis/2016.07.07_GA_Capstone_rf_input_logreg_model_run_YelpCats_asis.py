...
df = pd.read_csv('/home/jonathan/WinUbuntu/Python Scripts/Capstone-Nicole/modeling_data20160707/asis/df_Yelp_train.csv')
df2 = pd.read_csv('/home/jonathan/WinUbuntu/Python Scripts/Capstone-Nicole/modeling_data20160707/asis/df_Yelp_oos.csv')
...
rfc = RandomForestClassifier(criterion = 'gini', n_estimators = 80, 
...
# or better
features = ['yelp_reviews', 'time_diff', 'crime_count', 'avg_high_temp', 'insp_badge',
            'construction_count', 'yelp_rating', 'risk', 'hotels', 'burgers',
            'sandwiches', 'breakfast_brunch', 'mexican', 'tradamerican', 'hotdogs',
            'newamerican', 'bakeries', 'german']
...
def plot_learning_curve(X, Y):
    ...
    plt.figure()    
    plt.grid()
    plt.title("Learning Curve with Logistic Regression", size = 15)
...
#GridSearch for optimum parameters
PARAMETERS = {'C':[0.05,0.1,0.5,1,5,10,20,50,100,250,500,1000,2500,5000,7500,10000],
              'solver':['newton-cg','lbfgs', 'liblinear','sag'],
              'fit_intercept':[True,False], 'class_weight':['balanced',None]}
logreg_gridcv = GridSearchCV(logreg, PARAMETERS, verbose=True, n_jobs=-1, cv = 7)
logreg_gridcv.fit(X_train, Y_train)

print logreg_gridcv.best_estimator_
print logreg_gridcv.best_score_
...
plot_validation_curve(logreg, X_train, Y_train)


#Manual input of best parameter values from GridSearch, need to automate this input.
logreg = LogisticRegression(C = 0.05, solver= 'newton-cg', fit_intercept = True, 
                            class_weight = None)
logreg.fit(X_train, Y_train)
#logreg.fit(X_train_std, Y_train)
model_save = pickle.dumps(logreg)
Y_score_s = logreg.fit(X_train, Y_train).decision_function(X_test)
...
logreg2 = pickle.loads(model_save)
Y2_score_s = logreg2.decision_function(X2)
#Y2_score_s = logreg2.decision_function(X2_scaled)
Y2_pred_s = logreg2.predict(X2)
