...
LABELS=['insp_badge']
STANDARD_FEATURES=['yelp_rating', 'yelp_reviews', 'risk', 'crime_count', '311_count', 'construction_count', 'avg_high_temp', 'time_diff', 'prev_crit_viol', 'prev_non_crit_viol', 'prev_crit_viol_cos', 'prev_crit_viol_rpt', 'prev_non_crit_viol_cos', 'prev_non_crit_viol_rpt', 'prev_crit_viol_tbr', 'prev_non_crit_viol_tbr']
...
if __name__ == '__main__':
    ...
    evaluation = evaluation.append(pd.DataFrame(model_selection(train_data, ElasticNetCV(), BaggingClassifier(KNeighborsClassifier()), "ElasticNetCV", "BaggedKNeighborsClassifier")).T)
    evaluation = evaluation.append(pd.DataFrame(model_selection(train_data, LogisticRegressionCV(), BaggingClassifier(KNeighborsClassifier()), "LogisticRegressionCV", "BaggedKNeighborsClassifier")).T)
    evaluation = evaluation.append(pd.DataFrame(model_selection(train_data, LinearSVC(), BaggingClassifier(KNeighborsClassifier()), "LinearSVC", "BaggedKNeighborsClassifier")).T)
    evaluation = evaluation.append(pd.DataFrame(model_selection(train_data, SGDClassifier(), BaggingClassifier(KNeighborsClassifier()), "SGDClassifier", "BaggedKNeighborsClassifier")).T)
    evaluation = evaluation.append(pd.DataFrame(model_selection(train_data, RandomForestClassifier(), BaggingClassifier(KNeighborsClassifier()), "RandomForestClassifier", "BaggedKNeighborsClassifier")).T)
    ...
    evaluation = evaluation.append(pd.DataFrame(model_selection(train_data, ElasticNetCV(), ExtraTreesClassifier(), "ElasticNetCV", "ExtraTreesClassifier")).T)
    evaluation = evaluation.append(pd.DataFrame(model_selection(train_data, LogisticRegressionCV(), ExtraTreesClassifier(), "LogisticRegressionCV", "ExtraTreesClassifier")).T)
    evaluation = evaluation.append(pd.DataFrame(model_selection(train_data, LinearSVC(), ExtraTreesClassifier(), "LinearSVC", "ExtraTreesClassifier")).T)
    evaluation = evaluation.append(pd.DataFrame(model_selection(train_data, SGDClassifier(), ExtraTreesClassifier(), "SGDClassifier", "ExtraTreesClassifier")).T)
    evaluation = evaluation.append(pd.DataFrame(model_selection(train_data, RandomForestClassifier(), ExtraTreesClassifier(), "RandomForestClassifier", "ExtraTreesClassifier")).T)
    ...
    evaluation = evaluation.append(pd.DataFrame(model_selection(train_data, ElasticNetCV(), AdaBoostClassifier(), "ElasticNetCV", "AdaBoostClassifier")).T)
    evaluation = evaluation.append(pd.DataFrame(model_selection(train_data, LogisticRegressionCV(), AdaBoostClassifier(), "LogisticRegressionCV", "AdaBoostClassifier")).T)
    evaluation = evaluation.append(pd.DataFrame(model_selection(train_data, LinearSVC(), AdaBoostClassifier(), "LinearSVC", "AdaBoostClassifier")).T)
